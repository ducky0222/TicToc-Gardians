...
public:
	static IndexedTriangleList MakeTesselated(Dvtx::VertexLayout layout, int latDiv, int longDiv)
	{
		...
		const float lattitudeAngle = static_cast<const float>(3.14 / latDiv);
		const float longitudeAngle = static_cast < const float>(2.0f * 3.14 / longDiv);
		...
		for (int iLat = 1; iLat < latDiv; iLat++)
		{
			...
			{
				dx::SimpleMath::Vector3 calculatedPos;
