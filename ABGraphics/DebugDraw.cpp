...
namespace debugDraw
{
	...
	HRESULT Initialize(ID3D11Device* d3ddevice, ID3D11DeviceContext* d3dContext)
	{
		HRESULT hr = S_OK;
		g_States = std::make_unique<CommonStates>(d3ddevice);
		g_Batch = std::make_unique<PrimitiveBatch<VertexPositionColor>>(d3dContext);

		g_BatchEffect = std::make_unique<BasicEffect>(d3ddevice);
