...
namespace glas::Serialization
{
	...
	inline void SerializeJSonDefault(rapidjson::Value& jsonVal, const void* data, glas::TypeId type, RapidJsonAllocator& allocator)
	{
		...
		{
			auto& baseInfo = base.BaseId.GetInfo();
			auto& baseMembers = baseInfo.Members;

			for (auto& baseMember : baseMembers)
			{
				if (!baseMember.Variable.IsRefOrPointer())
				{
					rapidjson::Value MemberVal{ rapidjson::kObjectType };

					auto& baseMemberInfo = baseMember.Variable.GetTypeId().GetInfo();
					baseMemberInfo.JSonSerializer(MemberVal, VoidOffset(data, baseMember.Offset), allocator);

					jsonVal.AddMember(rapidjson::GenericStringRef{ baseMember.Name.c_str() }, MemberVal, allocator);
				}
			}
		}
	...
	inline void DeserializeJSonDefault(rapidjson::Value& jsonVal, void* data, glas::TypeId type)
	{
		...
		for (auto& base : info.BaseClasses)
		{
			...
			for (auto& baseMember : baseMembers)
			{
				if (!baseMember.Variable.IsRefOrPointer())
				{
					auto memberVal = jsonVal.FindMember(baseMember.Name);

					if (memberVal != jsonVal.MemberEnd())
					{
						auto& memberInfo = baseMember.Variable.GetTypeId().GetInfo();
						memberInfo.JSonDeserializer(memberVal->value, VoidOffset(data, baseMember.Offset));
					}
				}
			}
		}
